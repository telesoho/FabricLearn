...
infoln "Registering tls client id peer0-sdl on $CA_TLS_LOCALCOIN"
fabric-ca-client register -u https://$CA_TLS_LOCALCOIN --id.name peer0-sdl --id.secret peer0-sdlpw --id.type client  2>&1 1>&log.txt; ifErrorPause
