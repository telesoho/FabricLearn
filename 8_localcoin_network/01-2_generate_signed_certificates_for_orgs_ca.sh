...
fabric-ca-client register -u https://$ROOT_TLS_CA_SERVER --id.name ca-sdl-admin --id.secret ca-sdl-adminpw --id.type client 2>&1 1>&log.txt; ifErrorPause
...
--enrollment.profile tls --csr.hosts '*.sdl.localcoin.jp' 2>&1 1>&log.txt; ifErrorPause

mkdir -p $LOCAL_ROOT_PATH/fabric-ca-server/ca.sdl.localcoin.jp/tls/ca-sdl-admin
cp $FABRIC_CA_CLIENT_HOME/msp/signcerts/* $LOCAL_ROOT_PATH/fabric-ca-server/ca.sdl.localcoin.jp/tls/ca-sdl-admin/cert.pem ; ifErrorPause
cp $FABRIC_CA_CLIENT_HOME/msp/keystore/* $LOCAL_ROOT_PATH/fabric-ca-server/ca.sdl.localcoin.jp/tls/ca-sdl-admin/key.pem ; ifErrorPause
