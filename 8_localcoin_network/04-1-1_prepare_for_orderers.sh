...
cp $LOCAL_ROOT_PATH/config/orderer.yaml $FABRIC_CA_CLIENT_HOME/orderer.yaml 2>&1 1>&log.txt; ifErrorPause
...
configtxgen -profile OrgsOrdererGenesis -outputBlock $LOCAL_ROOT_PATH/fabric-ca-client/system-channel/genesis.block -channelID system-channel -configPath $LOCAL_ROOT_PATH/config 2>&1 1>&log.txt
